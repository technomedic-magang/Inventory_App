...
class Parameter extends MY_Controller
{
  public function __construct()
  {
    parent::__construct();
    // @model
    _models(['master/m_parameter']);
    // @table
    $this->table = $this->m_parameter->table;
    $this->pk_id = $this->m_parameter->pk_id;
    // @params
    $this->template = 'master/parameter/';
  }
  ...
  public function save($id = null)
  {
    ...
    if ($id == null) {
      if (DB::valid_id($this->table, @$this->pk_id, @$d[@$this->pk_id]) == true) {
        ...
        $d[@$this->pk_id] = DB::get_id(@$this->table);
        DB::insert($this->table, $d, $w);
        _json(_response('01', $this->uri));
        DB::update_id($this->table, $d[@$this->pk_id]);
      }
    } else {
  ...
  {
    $this->m_parameter->load_datatables();
  }
}
