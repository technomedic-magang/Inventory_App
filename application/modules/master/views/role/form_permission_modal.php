<form id="form" action="<?= $form_act ?>" method="post" autocomplete="off">
  <div class="card-body">
    <div class="mb-1 row">
      <label class="col-lg-2 col-md-6 col-form-label required">Role Id</label>
      <div class="col-lg-4">
        <input type="text" name="role_id" class="form-control" value="<?= @$main['role_id'] ?>" <?= @$main ? 'required' : '' ?> readonly>
      </div>
    </div>
    <div class="mb-1 row">
      <label class="col-lg-2 col-md-6 col-form-label required">Role</label>
      <div class="col-lg-8 col-md-6">
        <input type="text" name="role_nm" class="form-control" value="<?= @$main['role_nm'] ?>" readonly>
      </div>
    </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    tabel = $('#datatable-permission').DataTable({
      ...
      "ajax": {
        "url": "<?= $this->uri . '/ajax_datatables_permissions/' . @$main['role_id'] . '?n=' . _get('n') ?>",
        "type": "POST"
      },
      "deferRender": true,
