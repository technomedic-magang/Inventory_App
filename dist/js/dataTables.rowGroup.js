...
(function (factory) {
  ...
  $.extend(RowGroup.prototype, {
    ...
    _adjustColspan: function () {
      $("tr." + this.c.className, this.s.dt.table().body()).attr(
        "colspan",
        this._colspan()
      );
    },
    ...
    _draw: function () {
      ...
      for (var i = 0, ien = groupedRows.length; i < ien; i++) {
        ...
        if (this.c.startRender) {
          ...
          this._rowWrap(display, this.c.startClassName).insertBefore(
            firstRow.node()
          );
        }
        ...
        if (this.c.endRender) {
          ...
          this._rowWrap(display, this.c.endClassName).insertAfter(
            dt.row(group[group.length - 1]).node()
          );
        }
      }
    },
    ...
    _rowWrap: function (display, className) {
      ...
      if (
        ...
        row = $("<tr/>").append(
          $("<td/>").attr("colspan", this._colspan()).append(display)
        );
      }

      return row.addClass(this.c.className).addClass(className);
    },
  });
